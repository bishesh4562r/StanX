...
def load_bc_mask(n_entity):
    print('reading breast cancer related ids ...')
    bc_ids = pd.read_csv('../data/breast_related_ids.csv', header=0).iloc[:, 0].values
    bc_ids = bc_ids[bc_ids < n_entity]

    bc_mask = np.zeros(n_entity, dtype=np.float32)
    bc_mask[bc_ids] = 1.0
    return bc_mask
...
def load_data(args):
    ...
    return n_nodea, n_nodeb, n_entity, n_relation, adj_entity, adj_relation, bc_mask
