...
"""
class KG4SL(object):
    def __init__(self, args, n_entity, n_relation, adj_entity, adj_relation, bc_mask):
        self._parse_args(args, adj_entity, adj_relation)
        self.bc_mask_np = bc_mask
    ...
    def _build_model(self, n_entity, n_relation):
        self.bc_mask_tensor = tf.constant(self.bc_mask_np, dtype=tf.float32)  # [n_entity]
    ...
    def aggregate(self, entities, relations, embeddings_agg):
        ...
        entity_vectors = [tf.nn.embedding_lookup(self.entity_emb_matrix, i) for i in entities]
        relation_vectors = [tf.nn.embedding_lookup(self.relation_emb_matrix, i) for i in relations]
        bc_vectors = [tf.gather(self.bc_mask_tensor, i) for i in entities]  # NEW, rank-1 lookup
        for i in range(self.n_hop):
            ...
            for hop in range(self.n_hop - i):
                ...
                vector = aggregator(self_vectors=entity_vectors[hop],
                    ...
                    bc_indicator=tf.reshape(bc_vectors[hop + 1], [self.batch_size, -1, self.n_neighbor]),  # NEW
                    masks=None)
