...
class Aggregator(object):
    ...
    def __call__(self, self_vectors, neighbor_vectors, neighbor_relations, nodea_embeddings, bc_indicator, masks):
        return self._call(self_vectors, neighbor_vectors, neighbor_relations, nodea_embeddings, bc_indicator, masks)
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, nodea_embeddings, bc_indicator):
        ...
        # NEW: learnable additive bias toward BC-related neighbors
        if not hasattr(self, 'bc_weight'):
            self.bc_weight = tf.get_variable(self.name + '_bc_weight', shape=[],
                                            initializer=tf.constant_initializer(1.0))
        nodea_relation_scores = nodea_relation_scores + self.bc_weight * bc_indicator
...
class SumAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, nodea_embeddings, bc_indicator, masks):
        neighbors_agg = self._mix_neighbor_vectors(neighbor_vectors, neighbor_relations, nodea_embeddings, bc_indicator)
